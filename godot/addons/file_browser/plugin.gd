...
#  script_path: addons/file_browser/plugin.gd
#  module_name: plugin
#  version: 1.0.0
...
#  kind: module
#  spec: none
#  internal_dependencies: [addons/file_browser/file_browser.gd]
...
const FileBrowser := preload("res://addons/file_browser/file_browser.gd")
...
func _build_dock() -> Control:
	...
	_fields.tooltip_text = (
		...
		+ "Saved to fields.cfg and applied on the next pass."
	)
...
func _run(check_only: bool) -> void:
	...
	var cfg := FileBrowser.load_config(CONFIG_PATH)
	var files := FileBrowser.collect(root, [] as Array[String])
	var changed := 0
	var injected := 0
	var markers := 0
	var errors := 0
	...
	for path in files:
		var r: Dictionary = FileBrowser.process_file(path, cfg, check_only)
		if String(r.error) != "":
...
func _on_save_fields() -> void:
	...
	_log.clear()
	if err == OK:
		_log.append_text("Saved %d custom field%s to fields.cfg\n" % [
