...
#  MANIFEST
# =========================================================================
#  script_name: file_browser.gd
#  script_path: addons/file_browser/file_browser.gd
#  module_name: file_browser
#  version: 1.0.0
...
const CONFIG_PATH := "res://addons/file_browser/fields.cfg"
...
static func _divider(leader: String) -> String:
	var lead := leader + " "
	# Width counts the leader. `lead` is already "// " or "# ", so the run of
	# equals is the remainder, not the full width: getting this wrong by one
	# produced 76-character dividers from a constant that says 75.
...
## Path relative to the repository root, for the `script_path` field.
##
## An absolute path reaches files outside the Godot project, which is how the
## Rust crate is processed at all, but writing that absolute path into a header
## records this machine's directory layout in the source. Trimming back to the
## repo root keeps the field portable.
static func _repo_relative(p: String) -> String:
	var s := p.replace("\\", "/")
	for marker in ["/rust/", "/godot/", "/tools/", "/docs/"]:
		var i := s.rfind(marker)
		if i >= 0:
			return s.substr(i + 1)
	return s.trim_prefix("res://")


## First sentence of the file's own doc comment, used to seed `description`.
##
## A header whose description is blank says nothing, and the file usually
## already states its purpose in a `//!` or `##` line directly below where the
## header goes. Reusing it means the injected header is worth reading rather
## than a placeholder someone has to fill in later.
static func _doc_summary(text: String) -> String:
	for raw in text.split("\n"):
		var line := raw.strip_edges()
		var body := ""
		if line.begins_with("//!"):
			body = line.substr(3).strip_edges()
		elif line.begins_with("##"):
			body = line.substr(2).strip_edges()
		if body.is_empty():
			continue
		if body.begins_with("["):
			continue
...
static func _defaults_for(res_path: String, cfg: Config, existing: Dictionary) -> Dictionary:
	var rel := _repo_relative(res_path)
...
static func process_file(res_path: String, cfg: Config, check_only: bool = false) -> Dictionary:
	...
	values["script_path"] = _repo_relative(res_path)
	values["script_name"] = res_path.get_file()
	# Seed an empty description from the file's own doc comment rather than
	# leaving a blank field for someone to fill in later.
	if String(values.get("description", "")).strip_edges().is_empty():
		values["description"] = _doc_summary(body)
