        ...
        for i in 0..edge.geometry.len() - 1 {
            ...
            for nz in min_z..=max_z {
                for nx in min_x..=max_x {
                    if nx < 0 || nx >= width as i32 || nz < 0 || nz >= height as i32 { continue; }
                    let idx = nz as usize * width + nx as usize;
                    
                    let p_world = Vector3::new(nx as f32 - half_size.x, 0.0, nz as f32 - half_size.y);
                    ...
                    if dist < outer_radius {
                        ...
                        if dist < inner_radius {
                            ...
                            let weight = 1.0 - ((dist - inner_radius) / (outer_radius - inner_radius)).clamp(0.0, 1.0);
