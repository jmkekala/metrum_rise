...
pub(super) fn emit_disk(
    ...
    for step in 1..=sectors {
        ...
        push_triangle(mesh, layer, [center, previous, current], [center_uv, rim_uv, rim_uv], color);
        previous = current;
    }
}
...
fn circle_point(center: Vector3, radius: f32, angle: f32) -> Vector3 {
    Vector3::new(center.x + angle.cos() * radius, center.y, center.z + angle.sin() * radius)
}
