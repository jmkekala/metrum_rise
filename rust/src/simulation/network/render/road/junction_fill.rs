...
pub(super) fn edge_endpoint_trim_distance(
        ...
        && is_sidewalk_pass_through_node(graph, node_incidents, node_id)
    {
        if edge.primary_type == TransitType::Foot {
            return state.outer_radius.max(half_width);
        }
        return 0.0;
    }
    match state.kind {
...
pub(super) fn is_sidewalk_pass_through_node(
    ...
    let Some(d1) = direction_at_endpoint(edge_points(first_edge), road_incs[0].at_start) else {
        return false;
    };
    let Some(d2) = direction_at_endpoint(edge_points(second_edge), road_incs[1].at_start) else {
        return false;
    };
...
pub(super) fn emit_node_fill_polygon(
    ...
    for idx in 0..boundary.len() {
        let current = Vector3::new(
            boundary[idx].point.x,
            boundary[idx].point.y + y_offset,
            boundary[idx].point.z,
        );
        let next = Vector3::new(
            boundary[(idx + 1) % boundary.len()].point.x,
            boundary[(idx + 1) % boundary.len()].point.y + y_offset,
            boundary[(idx + 1) % boundary.len()].point.z,
        );
        push_triangle(
            mesh,
            layer,
...
pub(super) fn emit_polygon_fill(
    mesh: &mut NetworkMeshData,
    layer: MeshLayer,
    points: &[Vector3],
    y_offset: f32,
...
fn endpoint_section(points: &[Vector3], at_start: bool, trim: f32) -> Option<(Vector3, Vector2)> {
    let trimmed = if at_start {
        road_strip::trimmed_polyline(points, trim, 0.0)
    } else {
        road_strip::trimmed_polyline(points, 0.0, trim)
    };
    if trimmed.len() < 2 {
        return None;
    }
...
pub(super) fn emit_sidewalk_pass_through_aprons(
    mesh: &mut NetworkMeshData,
    graph: &RegionGraph,
    node_states: &HashMap<u32, NodeRenderState>,
    node_incidents: &HashMap<u32, Vec<IncidentEdgeEndpoint>>,
    ...
    for (&node_id, state) in node_states {
        ...
        let Some((road_incs, foot_incs)) = sidewalk_pass_through_components(graph, incidents)
        else {
            continue;
        };
...
pub(super) fn sidewalk_pass_through_components(
    graph: &RegionGraph,
    ...
    for incident in incidents {
        ...
        if road_supports_sidewalk(edge) {
            ...
            && (edge.allowed_types & TransitFlags::FOOT != 0)
        {
            foot_incs.push(*incident);
        } else {
            return None;
        }
    }
    if road_incs.len() != 2 || foot_incs.is_empty() {
        return None;
    }
    Some(([road_incs[0], road_incs[1]], foot_incs))
}

pub(super) fn sidewalk_apron_polygon(
    graph: &RegionGraph,
    node_id: u32,
    ...
    let ss = if foot_d.dot(road_norm) >= 0.0 {
        1.0
    } else {
        -1.0
    };
    ...
    let mut foot_pts = [
        lifted_offset(foot_c, foot_side, 0.0),
        lifted_offset(foot_c, -foot_side, 0.0),
    ];
...
fn sort_pair_by_axis(points: &mut [Vector3; 2], origin: Vector3, axis: Vector2) {
    if projected_dist_xz(points[0], origin, axis) > projected_dist_xz(points[1], origin, axis) {
        points.swap(0, 1);
    }
}
...
pub(super) fn pass_through_endpoint_side_override(
    graph: &RegionGraph,
    node_states: &HashMap<u32, NodeRenderState>,
    node_incidents: &HashMap<u32, Vec<IncidentEdgeEndpoint>>,
    edge: &Edge,
    ...
    let self_idx = incidents.iter().find_map(|inc| {
        if inc.at_start == at_start {
            ...
            if std::ptr::eq(candidate, edge) {
                return Some(inc.edge_idx);
            }
        }
        None
    })?;
    let other_inc = incidents.iter().copied().find(|inc| {
        inc.edge_idx != self_idx && graph.edges[inc.edge_idx].primary_type == TransitType::Road
    })?;
    ...
    let other_dir = direction_at_endpoint(
        edge_points(&graph.edges[other_inc.edge_idx]),
        other_inc.at_start,
    )?;
    let (prev_d, next_d) = if at_start {
        (-other_dir, self_dir)
    } else {
        (-self_dir, other_dir)
    };
...
pub(super) fn emit_width_transition_taper(
    ...
    let (n_inc, n_hw, w_hw) = if hw0 <= hw1 {
        (inc0, hw0, hw1)
    } else {
        (inc1, hw1, hw0)
    };
    ...
    let Some(dir) = direction_at_endpoint(pts, n_inc.at_start) else {
        return;
    };
    let Some((t_end, _)) = endpoint_section(pts, n_inc.at_start, taper_d) else {
        return;
    };
    ...
    push_quad(
        ...
        layer,
        [
            lifted_offset(center, perp * w_hw, y_offset),
            lifted_offset(t_end, perp * n_hw, y_offset),
            lifted_offset(t_end, perp * -n_hw, y_offset),
            lifted_offset(center, perp * -w_hw, y_offset),
        ],
        [uv, uv, uv, uv],
