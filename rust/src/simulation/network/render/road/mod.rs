...
impl TransitRenderer for RoadRenderer {
    fn generate_mesh_data(
        ...
        for (node_id, state) in &sidewalk_node_states {
            match state.kind {
                ...
                NodeRenderKind::Junction => {
                    if junction_fill::node_uses_polygon_fill(
                        graph,
                        &sidewalk_node_states,
                        &sidewalk_node_incidents,
                        *node_id,
                    ) {
                        junction_fill::emit_node_fill_polygon(
                            &mut mesh,
                            MeshLayer::Sidewalk,
                            graph,
                            *node_id,
                ...
                NodeRenderKind::WidthTransition => {
                    junction_fill::emit_width_transition_taper(
                        &mut mesh,
                        MeshLayer::Sidewalk,
                        graph,
                        *node_id,
        ...
        junction_fill::emit_sidewalk_pass_through_aprons(
            &mut mesh,
            graph,
            &sidewalk_node_states,
            &sidewalk_node_incidents,
