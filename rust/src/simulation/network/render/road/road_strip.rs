...
pub(super) fn edge_render_geometry(
    ...
    let start_trim = junction_fill::edge_endpoint_trim_distance(
        graph,
        node_states,
        node_incidents,
        edge,
    ...
    let end_trim = junction_fill::edge_endpoint_trim_distance(
        graph,
        node_states,
        node_incidents,
        edge,
    ...
    let start_side_override = if start_trim > 0.0 {
        ...
        junction_fill::pass_through_endpoint_side_override(
            graph,
            node_states,
            node_incidents,
            edge,
...
pub(super) fn clip_polyline_range(
    points: &[Vector3],
    start_distance: f32,
    end_distance: f32,
) -> Vec<Vector3> {
...
fn emit_marking_polyline(
    ...
    for segment in points.windows(2) {
        ...
        emit_marking_segment(
            mesh,
            start.lerp(end, local_a),
            start.lerp(end, local_b),
            lateral_offset,
            half_width,
            clip_start,
            clip_end,
