...
fn straight_width_change_is_not_rendered_as_junction_disk() {
    ...
    assert_eq!(
        node_states.get(&n1).map(|s| s.kind),
...
fn road_walkway_connection_keeps_sidewalk_pass_through() {
    ...
    assert_eq!(
        road_states.get(&n1).map(|s| s.kind),
        Some(NodeRenderKind::PassThrough)
    );
    assert_eq!(
        sidewalk_states.get(&n1).map(|s| s.kind),
        Some(NodeRenderKind::PassThrough)
    );
    assert!(!junction_fill::node_uses_polygon_fill(
        &graph,
        &sidewalk_states,
        &sidewalk_incs,
        n1
...
fn sidewalk_boundary_collapse_prefers_outer_points_on_shared_rays() {
    ...
    assert!(
        boundary
            .iter()
            .any(|p| p.point.distance_squared_to(Vector3::new(-5.0, 0.0, -5.0)) < 0.001)
    );
    assert!(
        boundary
            .iter()
...
fn sidewalk_pass_through_node_trims_walkway_but_not_road_shoulders() {
    ...
    let sidewalk_states = build_sidewalk_node_render_states(&graph, &sidewalk_incs);
    assert_eq!(
        junction_fill::edge_endpoint_trim_distance(
            &graph,
            &sidewalk_states,
            &sidewalk_incs,
            &graph.edges[0],
...
fn sidewalk_pass_through_node_builds_apron_polygon_for_centered_walkway() {
    ...
    let (road_incs, foot_incs) =
        junction_fill::sidewalk_pass_through_components(&graph, incs).unwrap();
    let polygon = junction_fill::sidewalk_apron_polygon(
        &graph,
        n1,
        &road_incs,
        foot_incs[0],
        *sidewalk_states.get(&n1).unwrap(),
    )
    .unwrap();
    for expected in [
        ...
        assert!(
            polygon
                .iter()
                .any(|p: &Vector3| p.distance_squared_to(expected) < 0.05)
        );
    }
}
