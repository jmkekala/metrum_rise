...
use super::backend::{
    ROAD_OVERLAY_COORDINATE_SCALE, RoadVec2, overlay_point_to_road, road_points_to_polyline,
    road_vec2_to_overlay_point,
};
use super::rails::{
...
fn split_non_road_regions(
    ...
    for kind in non_road_band_order() {
        ...
        let canonical_domains = canonical_same_band_contact_domains_before_final_boolean(
            &kind_target,
            &kind_domains,
            &rails.constraints,
        )?;
        let kind_result = if canonical_domains.is_empty() {
            owned_regions_from_domains(
                &kind_target,
                &kind_domains,
                &rails.constraints,
            ...
            let canonical_domain_refs = canonical_domains.iter().collect::<Vec<_>>();
            match owned_regions_from_domains(
                &kind_target,
                ...
                Ok(result) => result,
                Err(NodeBooleanOwnershipError::UnownedBandResidual {
                    kind: residual_kind,
                    ..
                }) if residual_kind == kind => owned_regions_from_domains(
                    &kind_target,
                    &kind_domains,
                    &rails.constraints,
                    ResidualKind::Band(kind),
...
fn canonical_same_band_contact_domains_before_final_boolean(
    target_shapes: &NodeOverlayShapes,
    domains: &[&NodeGeneratedContour],
    rail_constraints: &[NodeRailConstraint],
) -> Result<Vec<NodeGeneratedContour>, NodeBooleanOwnershipError> {
    if domains
        .iter()
        .any(|domain| band_kind(domain) != Some(RoadSurfaceBandKind::CurbOrShoulder))
    {
        return Ok(Vec::new());
    }

    let mut plan = owned_regions_from_domains(
        target_shapes,
        domains,
        rail_constraints,
        ResidualKind::Band(RoadSurfaceBandKind::CurbOrShoulder),
    )?;
    if plan.regions.len() < 2 {
        return Ok(Vec::new());
    }

    canonicalize_owned_region_rings(&mut plan.regions, target_shapes);
    append_shared_region_seam_constraints(&mut plan.regions, target_shapes, rail_constraints.len());
    if !resolve_same_band_role_crossing_joins(&mut plan.regions, rail_constraints) {
        return Ok(Vec::new());
    }
    canonicalize_owned_region_rings(&mut plan.regions, target_shapes);
    Ok(generated_domains_from_contact_plan(&plan.regions))
}

fn generated_domains_from_contact_plan(
    regions: &[NodeBooleanOwnedRegion],
) -> Vec<NodeGeneratedContour> {
    if regions
        .iter()
        .any(|region| region.shape.len() != 1 || region.kind != RoadSurfaceBandKind::CurbOrShoulder)
    {
        return Vec::new();
    }

    let mut domains = regions
        .iter()
        .filter_map(|region| {
            let points_xz = region.shape.first().map(|contour| {
                contour
                    .iter()
                    .copied()
            ...
            Some(NodeGeneratedContour {
                kind: NodeGeneratedContourKind::Band { kind: region.kind },
                ...
                backend_polyline: road_points_to_polyline(points_xz.iter().copied(), true),
                points_xz,
            })
        })
        .collect::<Vec<_>>();
    domains.sort_by_key(|domain| {
        (
            domain.claim_priority,
            domain.source_mouth_order_index,
            domain.source_band_index,
            domain.owner,
        )
    });
    domains
...
fn resolve_same_band_role_crossing_joins(
    ...
    for _ in 0..max_passes {
        ...
        if let Some(rewrite) = same_band_same_role_edge_rewrite_candidate(
            regions,
            rail_constraints,
            next_constraint_index,
        ) {
        ...
        if let Some(rewrite) =
            same_band_claim_priority_point_rewrite_candidate(regions, next_constraint_index)
        {
            apply_same_band_role_point_rewrite(regions, rewrite);
            next_constraint_index += 1;
            changed = true;
...
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd)]
struct SameBandSameRoleEdgeRewriteOrder {
    removed_role_priority: u8,
    donor_region_index: usize,
    receiver_region_index: usize,
    removed_key: NodeOwnershipPointKey,
    kept_key: NodeOwnershipPointKey,
    candidate_key: NodeOwnershipPointKey,
}

fn same_band_same_role_edge_rewrite_candidate(
    regions: &[NodeBooleanOwnedRegion],
    rail_constraints: &[NodeRailConstraint],
    new_constraint_index: usize,
    ...
    for left_index in 0..regions.len() {
        for right_index in left_index + 1..regions.len() {
            let left = &regions[left_index];
            let right = &regions[right_index];
            if left.kind != RoadSurfaceBandKind::CurbOrShoulder
                || right.kind != RoadSurfaceBandKind::CurbOrShoulder
            ...
            for (start, end) in shared_region_edges(left, right) {
                let Some(left_start_role) =
                    same_band_boundary_role_at_region_vertex(left, rail_constraints, start)
                else {
                    continue;
                };
                let Some(right_start_role) =
                    same_band_boundary_role_at_region_vertex(right, rail_constraints, start)
                else {
                    continue;
                };
                let Some(left_end_role) =
                    same_band_boundary_role_at_region_vertex(left, rail_constraints, end)
                else {
                    continue;
                };
                let Some(right_end_role) =
                    same_band_boundary_role_at_region_vertex(right, rail_constraints, end)
                else {
                    continue;
                };
                ...
                let Some(old_constraint_index) =
                    same_band_shared_constraint_index(left, right, start, end)
                else {
                    continue;
                };
                collect_same_band_same_role_edge_rewrite_candidates(
                    &mut candidates,
                    left_index,
                    right_index,
                    left,
    ...
    candidates.dedup_by_key(|(_, rewrite)| {
        (
            rewrite.donor_region_index,
            rewrite.receiver_region_index,
            rewrite.equal_key,
...
fn collect_same_band_same_role_edge_rewrite_candidates(
    candidates: &mut Vec<(SameBandSameRoleEdgeRewriteOrder, SameBandRoleJoinRewrite)>,
    donor_region_index: usize,
    receiver_region_index: usize,
    ...
    new_constraint_index: usize,
    start: (NodeOwnershipPointKey, SameBandBoundaryRole),
    end: (NodeOwnershipPointKey, SameBandBoundaryRole),
    rail_constraints: &[NodeRailConstraint],
) {
    for (removed, kept) in [(start, end), (end, start)] {
        let Some(candidate_key) = same_role_edge_endpoint_removal_candidate(
            donor,
            rail_constraints,
            removed.0,
            kept.0,
        ...
        if same_band_boundary_role_at_key(receiver, rail_constraints, candidate_key)
            != Some(removed.1)
        {
            continue;
        }
        candidates.push((
            SameBandSameRoleEdgeRewriteOrder {
                removed_role_priority: same_band_same_role_removed_endpoint_priority(removed.1),
                donor_region_index,
                receiver_region_index,
                removed_key: removed.0,
                kept_key: kept.0,
                candidate_key,
            },
            SameBandRoleJoinRewrite {
                donor_region_index,
                receiver_region_index,
                old_constraint_index: Some(old_constraint_index),
                new_constraint_index,
                equal_key: kept.0,
...
fn same_role_edge_endpoint_removal_candidate(
    region: &NodeBooleanOwnedRegion,
    rail_constraints: &[NodeRailConstraint],
    removed_key: NodeOwnershipPointKey,
    kept_key: NodeOwnershipPointKey,
    expected_role: SameBandBoundaryRole,
) -> Option<NodeOwnershipPointKey> {
    if same_band_boundary_role_at_region_vertex(region, rail_constraints, removed_key)
        != Some(expected_role)
    {
        return None;
    }
    ...
    for contour in &region.shape {
        ...
        for index in 0..keys.len() {
            ...
            if candidate_key == removed_key
                ...
                || triangle_double_area(kept_key, removed_key, candidate_key) == 0
            {
                continue;
            }
            if same_band_boundary_role_at_region_vertex(region, rail_constraints, candidate_key)
                == Some(expected_role)
            {
...
fn same_band_same_role_removed_endpoint_priority(role: SameBandBoundaryRole) -> u8 {
    match role {
        SameBandBoundaryRole::LowerSide => 0,
        SameBandBoundaryRole::RaisedSide => 1,
    }
}

fn same_band_claim_priority_point_rewrite_candidate(
    regions: &[NodeBooleanOwnedRegion],
    new_constraint_index: usize,
) -> Option<SameBandRolePointRewrite> {
    for left_index in 0..regions.len() {
        for right_index in left_index + 1..regions.len() {
            ...
            let (donor_index, donor) = if left.claim_priority != right.claim_priority {
                if left.claim_priority > right.claim_priority {
                    (left_index, left)
                } else {
                    (right_index, right)
                }
            } else if left.owner.owner_index() < right.owner.owner_index() {
                (left_index, left)
            } else {
                (right_index, right)
            };
            let shared_edges = shared_region_edges(left, right);
            for key in shared_region_points(left, right) {
                if shared_edges
                    .iter()
                    .any(|(start, end)| *start == key || *end == key)
                {
                    continue;
                }
                if let Some((previous_key, next_key)) =
                    removable_claim_priority_contact_point(donor, key)
                {
                    return Some(SameBandRolePointRewrite {
                        donor_region_index: donor_index,
                        new_constraint_index,
                        previous_key,
                        conflict_key: key,
                        next_key,
...
fn removable_claim_priority_contact_point(
    region: &NodeBooleanOwnedRegion,
    conflict_key: NodeOwnershipPointKey,
