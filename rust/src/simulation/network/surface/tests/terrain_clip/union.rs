...
fn terrain_clip_union_exports_hole_contours_for_ring_footprint() {
    ...
    let rectangles = [
        ...
        vec![
            RoadVec3::new(0.0, y, 3.0),
            RoadVec3::new(4.0, y, 3.0),
            RoadVec3::new(4.0, y, 4.0),
            RoadVec3::new(0.0, y, 4.0),
        ],
        vec![
            RoadVec3::new(0.0, y, 1.0),
            RoadVec3::new(1.0, y, 1.0),
            RoadVec3::new(1.0, y, 3.0),
            RoadVec3::new(0.0, y, 3.0),
        ],
        vec![
    ...
    assert!(
        hole_loop.points_world.iter().all(|point| {
            point.x >= 1.0 - f64::from(SAMPLE_EPSILON_M)
                && point.x <= 3.0 + f64::from(SAMPLE_EPSILON_M)
                && point.z >= 1.0 - f64::from(SAMPLE_EPSILON_M)
                && point.z <= 3.0 + f64::from(SAMPLE_EPSILON_M)
        }),
...
fn terrain_clip_union_skips_same_key_dust_only_when_degenerate() {
    ...
    let p0_same_key = RoadVec3::new(0.0000002, y + 0.25, 0.0000002);
    ...
    assert!(
        unioned[0]
            .source_edges
            .iter()
            .all(|edge| SurfaceXzKey::from_world_xz(edge.start)
                != SurfaceXzKey::from_world_xz(edge.end)),
...
fn terrain_clip_union_preserves_boundary_only_connector_by_interpolation() {
    ...
    assert!(
        clip_export.loops[0]
            .points_world
            .iter()
            .all(|point| (point.y - raw_boundary_y).abs() > f64::from(SAMPLE_EPSILON_M)),
