//! Source-edge endpoint footprint authorization.
...
pub(super) fn terminal_source_edge_endpoints_authorize_footprint_height_pair(
    ...
    source_edges.iter().any(|lower_edge| {
        source_edge_endpoint_proves_candidate_at_key(lower_edge, key, lower)
            && source_edges.iter().any(|raised_edge| {
                source_edge_endpoint_proves_candidate_at_key(raised_edge, key, raised)
            })
    })
}

fn source_edge_endpoint_proves_candidate_at_key(
...
fn source_edge_endpoint_matches_candidate(
