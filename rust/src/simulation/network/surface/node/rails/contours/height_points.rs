...
pub(in crate::simulation::network::surface::node::rails) fn align_height_points_to_contour(
    ...
    for point in source_points_world {
        ...
        if let Some(existing_height_key) = height_by_key.get(&key)
            && SurfaceHeightMmKey::from_m_f64(*existing_height_key) != height_key
        {
            return None;
        }
        height_by_key
            .entry(key)
            .and_modify(|height| *height = height.min(point.y))
            .or_insert(point.y);
    }
    contour_points_xz
        .iter()
...
fn height_on_source_contours(
    ...
    for source_contour_world in source_contours_world {
        if let Some(candidate_height_m) = height_on_source_contour_edge(key, source_contour_world) {
            ...
            if let Some(existing_height) = height
                && SurfaceHeightMmKey::from_m_f64(existing_height) != candidate_height_key
            {
                return None;
            }
            height = Some(height.map_or(candidate_height_m, |current| {
                current.min(candidate_height_m)
            }));
        }
    }
    height
...
pub(in crate::simulation::network::surface::node::rails) fn height_for_key_on_generated_edge(
    ...
    // Millimetre keys validate agreement; they must not round the geometry itself.
    Some(start_height_m + (end_height_m - start_height_m) * parameter.as_f64())
}
