    ...
    pub fn split_edge(&mut self, edge_idx: usize, split_pos: Vector3) -> (u32, usize) {
        ...
        if old_edge.physical_geometry.is_empty() {
            ...
            for i in phys_split_idx + 1..old_edge.physical_geometry.len() {
                if i == phys_split_idx + 1
                    && old_edge.physical_geometry[i].distance_to(phys_closest) < 0.01
                {
                    continue;
                }
        ...
        self.edges[edge_idx].physical_length =
            self.calculate_length(&self.edges[edge_idx].physical_geometry);
        ...
        self.edges[new_edge_id].physical_length =
            self.calculate_length(&self.edges[new_edge_id].physical_geometry);
        ...
        if !fwd_tgts.is_empty() {
            frontage_conns.insert((edge_idx, 0), fwd_tgts);
        }
        ...
        if !bkw_tgts.is_empty() {
            frontage_conns.insert((new_edge_id, -1), bkw_tgts);
        }
