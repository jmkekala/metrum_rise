...
//           network structure. Edges are held in an append-only vector
//           with a deleted flag rather than being removed, so cached
//           indices held by agents and buildings stay valid until an
//           explicit compaction remaps them. Nodes merge through a
//           union-find, which keeps a removed node id resolvable to its
//           canonical survivor. The R-tree index over edge AABBs answers
//           the spatial queries editing and placement issue every frame.
//  kind: module
//  spec: none
