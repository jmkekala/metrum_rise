...
use super::{
    ...
    optional_edge_to_db, usize_to_i64,
};
        ...
        while let Some(row) = rows.next()? {
            ...
            push_loaded_agent(
                ...
                LoadedAgentRecord {
                    ...
                    current_edge: db_to_optional_usize(row.get(8)?)?,
                    current_lane_id: row.get(9)?,
                    lane_distance: row.get(10)?,
                    pos_x: row.get(11)?,
                    pos_y: row.get(12)?,
                    is_visible: row.get(13)?,
                    activity: i64_to_u8(row.get(14)?)?,
                    transit: i64_to_u8(row.get(15)?)?,
                    transit_mode: i64_to_u8(row.get(16)?)?,
                    happiness: row.get(18)?,
                    money: row.get(19)?,
                    journey_start_time: row.get(20)?,
                    has_car: row.get(21)?,
                    vehicle_type: i64_to_u8(row.get(22)?)?,
                    current_path_index: i64_to_usize(row.get(23)?)?,
...
fn clear_agent_travel_state(agents: &mut AgentSystem, i: usize) {
    ...
    } else if agents.transit[i] != TRANSIT_IDLE && agents.transit[i] != TRANSIT_IMMIGRATING {
        agents.transit[i] = TRANSIT_ON_ROAD;
    }
}
