...
use super::schema::*;
use super::{i64_to_u32, i64_to_usize, i64_to_i8, usize_to_i64};

pub(super) fn save_network(tx: &Transaction, graph: &RegionGraph, maps: &SnapshotMaps) -> SaveLoadResult<()> {
    // 1. Nodes
    {
        let mut node_rows: Vec<(u32, u32)> = maps.node_old_to_new.iter().map(|(&old, &new)| (new, old)).collect();
    ...
    // 3. Geometry
    {
        ...
        for (old_id, edge) in graph.edges().iter().enumerate() {
            ...
            for (idx, p) in edge.physical_geometry.iter().enumerate() {
                stmt.execute(params![usize_to_i64(saved_id)?, usize_to_i64(idx)?, p.x, p.y, p.z, true])?;
            }
        }
    }
        ...
        for (&old_node, &saved_node) in &maps.node_old_to_new {
            ...
            for (&(from_e, from_l), targets) in &node.lane_connections {
                ...
                for &(to_e, to_l) in targets {
                    let Some(&saved_to) = maps.edge_old_to_new.get(&to_e) else { continue; };
                    stmt.execute(params![i64::from(saved_node), usize_to_i64(saved_from)?, i64::from(from_l), usize_to_i64(saved_to)?, i64::from(to_l)])?;
                }
            }
        }
    }
...
pub(super) fn load_graph(conn: &Connection) -> SaveLoadResult<RegionGraph> {
    ...
    let (mut geometry, mut physical_geometry) = (HashMap::new(), HashMap::new());
    {
        ...
        while let Some(row) = rows.next()? {
            ...
            if row.get::<_, bool>(5)? { physical_geometry.entry(eid).or_insert_with(Vec::new).push(p); }
            else { geometry.entry(eid).or_insert_with(Vec::new).push(p); }
        }
    }

    {
        ...
        while let Some(row) = rows.next()? {
            ...
            graph.add_edge(Edge {
                ...
                allowed_types: (row.get::<_, i64>(4)?) as u8, class: edge_class_from_i64(row.get(5)?)?, width: row.get(6)?,
                fwd_lanes: (row.get::<_, i64>(7)?) as u8, bkw_lanes: (row.get::<_, i64>(8)?) as u8, speed_limit: row.get(9)?,
