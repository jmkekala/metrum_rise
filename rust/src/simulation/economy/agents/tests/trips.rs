...
fn test_agent_departure_car_selection() {
    ...
    agents.spawn_border_arrival_agent(0, n0, 100.0, 0.0);
    let a_id = 0;
...
fn test_simultaneous_car_egress_queues_instead_of_deadlocking_at_attach_point() {
    ...
    for _ in 0..5 {
        let a_id =
            agents.spawn_border_arrival_agent(0, n0, entrance.door_pos.x, entrance.door_pos.y);
        agents.transit[a_id] = TRANSIT_ACCESS_EGRESS;
        agents.transit_mode[a_id] = MODE_CAR;
        agents.current_building[a_id] = 0;
        agents.pos_x[a_id] = entrance.door_pos.x;
...
fn test_simultaneous_car_ingress_queues_instead_of_deadlocking_at_detach_point() {
    ...
    let mut ids = Vec::new();
    for _ in 0..5 {
        let a_id = agents.spawn_border_arrival_agent(0, n0, lane_point.x, lane_point.y);
...
fn test_planned_departure_populates_exact_trip_plan() {
    ...
    for has_car in [false, true] {
        ...
        if !has_car {
            ...
            let lane = agents.planned_attach_lane_id[i] as usize;
            assert_eq!(
                network.lane_system.lanes[lane].lane_type,
...
fn test_same_edge_direct_car_trip_reaches_opposite_side_destination_building() {
    check_same_edge_opposite_side_arrivals(1, 2000);
}

#[test]
fn test_same_edge_opposite_side_household_car_arrivals_eventually_finish_ingress() {
    check_same_edge_opposite_side_arrivals(5, 10000);
}

fn check_same_edge_opposite_side_arrivals(count: usize, max_ticks: usize) {
    let mut g = RegionGraph::new();
    let n0 = g.add_node(Vector3::new(0.0, 0.0, 0.0), NodeType::Junction);
    let n1 = g.add_node(Vector3::new(500.0, 0.0, 0.0), NodeType::Junction);
    g.add_edge(Edge {
...
fn test_vehicle_type_persistence() {
    let mut agents = AgentSystem::new();
    let _i0 = agents.spawn_border_arrival_agent(usize::MAX, 0, 0.0, 0.0);
    let _i1 = agents.spawn_border_arrival_agent(usize::MAX, 0, 0.0, 0.0);
    let i2 = agents.spawn_border_arrival_agent(usize::MAX, 0, 0.0, 0.0);
    let type2 = agents.vehicle_type[i2];
    let mut allocator = BuildingAllocator::new();
    let _ = agents.kill_agent(
        1,
        &mut allocator,
...
fn test_border_spawn_movement() {
    ...
    allocator.buildings.push(home);
    allocator.rebuild_entrance_cache(&graph, &network.lane_system);
    let agent_idx = agents.spawn_border_arrival_agent(0, n1, 100.0, 0.0);
...
fn test_border_freight_replans_after_empty_path_freeze() {
    ...
    let agent_idx =
        agents.spawn_border_arrival_agent(usize::MAX, start_node, start_pos.x, start_pos.z);
...
fn test_pedestrian_crosses_junction() {
    ...
    graph.add_node(Vector3::new(0.0, 0.0, 0.0), NodeType::Junction);
    graph.add_node(Vector3::new(100.0, 0.0, 0.0), NodeType::Junction);
    let mut zoning = ZoningSystem::new(&WorldConfig::default());
    let mut allocator = BuildingAllocator::new();
    network.add_road(
        &mut graph,
