...
fn bkw_vehicle_lanes(network: &TransitNetwork, edge_idx: usize) -> Vec<usize> {
    network.lane_system.edge_lanes[&edge_idx]
        .iter()
        .filter(|&&lid| {
            let l = &network.lane_system.lanes[lid];
...
fn assert_connection_lane_spacing(
    agents: &AgentSystem,
    network: &TransitNetwork,
    tick: usize,
    label: &str,
...
fn check_connection_spacing_two_edge(fwd: u8, bkw: u8, label: &str) {
...
fn check_connection_spacing_4way(fwd: u8, bkw: u8, label: &str) {
