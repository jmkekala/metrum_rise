...
pub struct SimCore {
    ...
    /// Latest asynchronous road-tool preview compiled off the Godot main thread.
    pub(crate) latest_road_preview: Option<RoadPreviewSnapshot>,
...
pub struct RenderSnapshot {
    ...
    /// Latest asynchronous road-tool preview result, if one has completed.
    pub(crate) road_preview: Option<RoadPreviewSnapshot>,
}
...
pub enum SimCommand {
    ...
    /// Compile a road-tool preview on the background thread and publish it through the snapshot.
    CompileRoadPreview {
        /// Monotonic request id generated by the Godot bridge.
        request_id: u64,
        /// World-space preview centerline points.
        points: Vec<godot::prelude::Vector3>,
        /// Forward lane count.
        fwd_lanes: i32,
        /// Backward lane count.
...
impl SimCore {
    ...
    pub fn build_snapshot(&self) -> RenderSnapshot {
        ...
        RenderSnapshot {
            ...
            road_preview: self.latest_road_preview.clone(),
        }
    }
}
...
pub fn run_sim_thread(
    ...
    loop {
        ...
        let mut pending_road_preview: Option<(u64, Vec<Vector3>, i32, i32)> = None;
        loop {
            match cmd_rx.try_recv() {
                ...
                Ok(SimCommand::CompileRoadPreview {
                    request_id,
                    points,
                    fwd_lanes,
                    bkw_lanes,
                }) => {
                    pending_road_preview = Some((request_id, points, fwd_lanes, bkw_lanes));
                }
        ...
        if let Some((request_id, points, fwd_lanes, bkw_lanes)) = pending_road_preview {
            ...
            let preview =
                core.compile_road_preview_snapshot(request_id, points, fwd_lanes, bkw_lanes);
            ...
            core.latest_road_preview = Some(preview);
            if road_debug {
                debug_log!(
                    "road",
                    "preview_surface_async request_id={} points={} prepared_points={} surface_vertices={} valid={} total_ms={:.3}",
                    request_id,
                    point_count,
                    prepared_count,
