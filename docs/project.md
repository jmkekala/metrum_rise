...
57. **Asset editor / importer**:
    ...
    - Canonical content format: `glTF 2.0` / `.glb` plus human-authored TOML manifests (`pack.toml`, `asset.toml`). The editor may accept FBX as a source convenience, but shipped assets should be normalized to `.glb` outputs and explicit metadata files written into the exported pack folder.
    - Pack distribution should be simple: creators may share exported packs as normal folders or `.zip` archives, but the installed runtime form should be an extracted pack directory in the mod folder, not a pile of ad-hoc loose files and not zip-streaming as the primary loading path.
    - Export should support both an unpacked pack folder and a share archive. Default integrity tooling should generate SHA-256 hashes for the archive and unpacked contents (`checksums.sha256`) so import/load can detect corruption or local modification. If trusted anti-tamper/authorship is desired later, add signatures on top of hashes rather than treating bare hashes as a security feature.
    - First milestone scope: zoned buildings, props, vehicles, and character source assets for offline baking. Roads are a later dedicated network-authoring milestone because they must define lane/topology/material metadata, not just meshes.
    - First implementation blockers are now explicit: lock the v1 pack scanner rules, `pack_id` / `asset_id` grammar, per-class `asset.toml` schema, canonical pack layout, and the minimum per-class preview/inspector interactions before writing loader or editor code against guesswork.
    - Add pack/set manifests with enable-disable support, author/license attribution, standardized editor-generated thumbnails, and per-asset metadata (variable lot width/depth in zoning cells, zone type, service class, worker/resident capacity, category, tags, LODs, source attribution). `3×3` remains only a preset/default, not a hard rule.
    - `asset_id` should be required in the very first manifest version even before saves depend on it directly. Stable save-facing IDs are cheap to add now and expensive to retrofit later.
    - LOD content should be exported as part of the pack, not regenerated by a Godot project import step on the mod user's machine. Authors may provide custom LOD meshes, and the asset editor may generate draft fallback meshes offline, but the editor must preview/validate/package the approved LOD chain into the final pack. Default runtime behavior should stay discrete + hysteresis rather than global cross-fade blending; silhouette matching is the primary anti-pop requirement, with optional landmark-only exceptions later if justified.
    - Default asset budgets should target an `8 GB VRAM` floor and roughly `12-16 GB VRAM` recommended hardware. The editor should surface triangle, material, texture, and estimated VRAM costs, with especially strict defaults for VAT characters because their runtime cost scales with visible vertex count.
    - Asset budgets should explicitly cover shadow proxies, far-traffic impostors, and foliage as a separate high-risk class. Long roads should not go visually dead from aggressive traffic culling, and dense tree placement must transition to billboard/impostor rendering early.
    - Later ecosystem work, not a blocker for the first editor build: large-library manifest caches, compatibility/dependency metadata, redirect tables, cross-pack resource/library packs, external workspace metadata, and signing. Keep the design notes, but do not block the first loader/importer implementation on them.
    - Pack layout should be category-first for human browsing (`vehicles/civil/...`, `vehicles/police/...`, `vehicles/fire/...`, `buildings/residential/...`) rather than a flat list of opaque folders like `sedan_01`, `sedan_02`, etc. Stable identity comes from `asset_id` inside `asset.toml`, not from the folder name alone.
    - Global uniqueness should come from `pack_id:asset_id`, not from forcing every folder slug in every pack to be globally unique. Multiple packs may each ship a `building.residential.lowrise_corner`; only uniqueness within a single pack is required.
    - Asset packs should remain data-only by default: no arbitrary scripts, native plugins, or unrestricted shader code as the normal mod surface. Materials should flow through an engine-controlled schema so the runtime stays portable, safe, and performance-bounded.
    - Asset metadata should include anchors/bounds where relevant (for example `[[anchors]]` entries for building entrance/frontage, service points, and prop sockets plus editable render/selection bounds and shadow proxies) so the engine does not have to guess interaction points from arbitrary visual meshes.
    - Civilian characters should be authored as reusable archetype families (`adult_male`, `adult_female`, `child`) that share rest meshes/VAT data within the family and vary mostly by swappable skin/clothing textures. Children should remain a separate archetype family rather than being forced onto adult assets.
    - Character source animations should be shared across archetypes where possible (`walk`, `idle` as the common base set), with retargeting/baking producing separate runtime outputs only when mesh proportions or vertex layouts require it.
    ...
    - For building authoring UX, the editor should offer a simple `Set Front From Current View` / `Use Camera-Facing Side As Frontage` action as the initial guess, but the exported asset must still store an explicit confirmed frontage/entrance direction rather than tying frontage to a live camera rule.
    ...
    - Shipped mod packs must load at runtime from the canonical external mod directory `user://mods/`, not from project-only `res://assets/...` paths. Rust owns manifests/metadata; Godot runtime owns mesh/texture/material loading. Mod users should not need the Godot Editor, Rust, or source code installed.
    - Replace the current building `(zone_type, variant)` pipeline (`NUM_VARIANTS`, `model_metadata.json`, directory scans under `res://assets/models/buildings/...`) with a registry-driven asset lookup keyed by manifest-backed IDs. Runtime building instances and save/load plumbing cannot stay `variant: u8`-only if imported packs are meant to survive save/load as real assets. Until that changes, exported packs cannot be addressed or rendered as real building assets.
    - Normalize the runtime building scale/pivot contract so editor-authored meshes use the same canonical units and placement pivot in preview and in-game. The current global `BUILDING_VISUAL_SCALE` compatibility factor is acceptable for bundled legacy content, but it is not a trustworthy long-term contract for imported mod assets.
    - VAT baking in the shipped editor must be self-contained for the user: no required Blender install, no required Python scripts, no Godot Editor dependency. That does not force a pure-GDScript implementation; a bundled Rust/native bake backend is acceptable and likely preferable for heavy skinning/export work. A separate bundled helper executable remains allowed, but it is the highest packaging-cost option because it adds another per-platform artifact. Existing `tools/bake_vat*.py` scripts should remain transitional developer tooling until the replacement reaches output parity, where "parity" means matching orientation/vertex-ID contract plus visually indistinguishable preview motion and baked vertex deltas within a defined epsilon rather than byte-for-byte identical floats.
    - Character import must respect the rendering roadmap: skeletal source assets are allowed in the editor, but runtime outputs must remain crowd-scale friendly (static, billboard, or VAT-baked), not live per-instance skeleton animation.
...
58. **Dynamic zoning extents / arbitrary lot sizes**:
    ...
    - Realistic scope: the core storage conversion is likely `1-2` focused days, followed by a full caller audit across split/merge, obstruction, zoning paint UI, queries, rendering helpers, allocator fit checks, and save/load plumbing. This is the true cost center behind arbitrary lot sizes.
