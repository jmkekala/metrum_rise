...
- **Graph-dilation road renderer**: `RoadRenderer` no longer infers custom junction polygons from edge-local offsets. It now follows the `graph-road-renderer` model directly: emit widened sidewalk and asphalt ribbons from the road centerline, add circular node fills only at true junctions and terminals, and keep edge curvature continuous with connected polyline strips rather than per-sample circular caps. This removes the old contour solver and all edge-to-node handoff clipping from the active top-surface path.
