...
- **Area-based road renderer**: `RoadRenderer` now treats every true multi-road junction as a node-owned corridor-union solve. For each junction, the renderer builds exact `road corridor` and `outer corridor` polygons, extracts the exact union contours, triangulates the asphalt contour, and emits sidewalk only on the exposed outer hull as `outer - road`. Edge ribbons no longer restart from one shared scalar envelope trim; each emitted road or sidewalk strip is clipped against the matching exact node contour, so obtuse bends and arbitrary-angle merges can meet the node footprint without gaps or sidewalk bleed.
- **Node sidewalk emission**: junction sidewalks are no longer emitted as fixed quads or as a hole-bridged fallback polygon. Each exact outer-minus-road sector is triangulated directly from the real contour pair, and node sidewalk triangles now use the same `UV.y` contract as edge sidewalks (`0 = road edge`, `1 = outer edge`). That keeps curb shading consistent across the edge-to-node handoff and prevents skewed merge bands from cutting white triangles across asphalt interiors.
- **Seam and shallow-angle continuity**: split straight roads stay coplanar without a blank center island, arbitrary-angle bends/merges are resolved by the same exact corridor-union ownership rule instead of pairwise edge stitching or raster-snapped contours, and 4-way nodes stay inside the actual road footprint instead of inflating into diagonal lobes. This removes the visible road caps, broken curb joins, straight-road bubbles, and arbitrary-angle junction artifacts that came from the old edge-local corner logic.
...
- **Junction Rendering (Item 70)**: The old corner-intersection / radial-fan junction builder is gone from the active path. Junction ownership now comes from exact union contours built from trimmed road corridor polygons, with asphalt filled from the road contour and sidewalks emitted only from the exposed `outer - road` band. Straight pass-through splits remain pure edge ribbons, while arbitrary-angle bends, T-junctions, shallow merges, and 4-way nodes all render through the same node-local ownership contract. The edge pass now clips each lane or sidewalk strip against the exact road or outer loop for its lateral band, which prevents the obtuse-angle gap/overlap failures that a single envelope trim could not represent.
