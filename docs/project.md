...
- **Scale target**: at least 20,000,000 total agents across multiple cities, regions, and simulation tiers in one large world.
- **Closest competitor on scale**: Cities: Masterplan, which claims 1:1 scale for every building, power plant, airport, vehicle, and car. See the prior-art index in [`reference.md`](reference.md).
...
- `PORT-01`: the physical layer port, at `P0`. Water that flows, fire with a fuel bed, wind, minerals
   derived from formation, flora, and fauna as one combined field. Everything built before it assumes
   still-water fills and absent fire. See [`simulation_layers.md`](simulation_layers.md).
- `GYM-01`: a gym, zoo, and museum for every system. The spikes under `godot/spike_*.gd` are the raw material.
   See [`README2-ONBOARDING.md`](README2-ONBOARDING.md).
