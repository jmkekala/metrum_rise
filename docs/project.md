...
- FSM state names (entrance/exit Phase 3): `IN_BUILDING → ACCESS_EGRESS → NETWORK → ACCESS_INGRESS → IN_BUILDING` + `IMMIGRATING` and `INTERSECTION`. The live movement loop now follows the entrance/exit model from `docs/entrance_and_exit.md` for ordinary trips. The only intentional no-plan state that remains is the `IMMIGRATING` bootstrap before its first exact trip build.
...
- **Building road attachment**: buildings still store authoritative `(edge_idx, frontage_t: f32, side, cell_x, cell_y)`. `building_depart_node(building, graph)` still exists for non-agent systems that have not yet been redesigned around entrances, but normal agent trips no longer use it as their primary building/network abstraction. The live exact-plan path now comes from the derived `BuildingEntrance` cache: `BuildingAllocator` computes `door_pos`, `curb_pos`, legal frontage lanes, and `vehicle_frontage_access`-aware access metadata from the asset-authored `entrance/main` anchor plus live lane topology. `TRANSIT_ACCESS_EGRESS` and `TRANSIT_ACCESS_INGRESS` consume that cache and the exact `planned_attach_*` / `planned_detach_*` fields instead of rebuilding curb/frontage heuristics per tick. No virtual split node is inserted into the road graph. `frontage_node` column removed from the `buildings` SQLite table. Detailed teardown, migration plan, and final target behavior: [`docs/entrance_and_exit.md`](entrance_and_exit.md).
- **Agent trip-plan SoA fields (2026-04-08, entrance/exit Phase 3)**: `AgentSystem` now carries the new compact trip/access scalars from [`docs/entrance_and_exit.md`](entrance_and_exit.md): `planned_attach_node`, `planned_detach_node`, `planned_attach_lane_id`, `planned_detach_lane_id`, `planned_attach_lane_d`, `planned_detach_lane_d`, `access_flags`, and `next_replan_time`. Save/load persists these columns with additive `ALTER TABLE agents ADD COLUMN ...` migration so older saves default the new fields to invalid-plan sentinels. `target_node` and `is_visible` remain temporarily because the live FSM, renderer, and save validation still have legacy readers.
...
- **Stuck Agent Scrub**: `AgentSystem::tick` now includes a safety pass that hides (`is_visible = false`) any agents that enter an `IN_BUILDING`-equivalent state while outside of a building, effectively cleaning up "field-ghost" artifacts from failed pathfinding.
...
#### Agent Rules
- **Immigration**: if at least one connected `NodeType::Border` exists, the allocator admits explicit immigrant households through those border nodes. Each admitted household claims real vacant housing first, then one resident agent per household member spawns at the border and travels inward with a shared `household_id`.
