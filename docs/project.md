...
- **Graph-dilation road renderer**: `RoadRenderer` no longer infers custom junction polygons from edge-local offsets. It now follows the `graph-road-renderer` model directly: emit a widened sidewalk-colored base from road-centerline strips, add circular node fills at true junctions and terminals, then draw the asphalt surface on top from the same strips and node disks. Interior polyline join disks are emitted only for real XZ bends, not for straight graded resample points, so uphill/downhill roads do not become stacked circular caps. This removes the old contour solver and all edge-to-node handoff clipping from the active top-surface path.
...
- **Node classification**: terminal nodes receive round caps, true multi-road junctions receive circular road fills sized by the widest incident edge, and straight two-edge pass-through splits skip the node disk when the widths match. That keeps straight continuations continuous without reintroducing the old arbitrary-angle junction logic.
