...
- **Economy foundation**: household records, building-centric daily economy, physical truck freight jobs, `OWA` fallback, exact entrance-side freight routing/ETA, unemployment benefit disbursement, two-day building bankruptcy, short private-building construction timers, and baseline fiscal revenue (income tax, household/business purchase tax, business profit tax, and construction property tax) are all live. See [`economy.md`](economy.md).
...
- **Persistence and runtime**: SQLite save/load, background simulation thread, render snapshots, debug flags, asset editor, and economy editor are live. The asset editor now supports multi-part building assets, driveway/parking/loading-bay site anchors, and authored polygon yard surfaces for textured asphalt and concrete. Yard surfaces are editor-preview metadata only for now; live terrain-integrated yards are parked as `EARTH-02`, whose target is a flat whole-lot building-site client registered at construction start, with zoning kept terrain-neutral. Vehicle parking / freight stop behavior remain later runtime hooks.
