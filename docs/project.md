...
- **Persistence and runtime**: SQLite save/load, background simulation thread, render snapshots, debug flags, asset editor, and economy editor are live. The asset editor now supports multi-part building assets plus authored driveway, parking, and loading-bay site anchors as metadata for later yard/vehicle-access runtime hooks.
