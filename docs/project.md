...
| B10 | [BUG] | `simulation/economy/agents/tick.rs` | Cars stack on top of each other at junctions and at road nodes. No intersection priority rules exist: any number of cars can be simultaneously in `TRANSIT_INTERSECTION` and enter the same junction lane without yielding. Root causes: (1) no per-junction capacity or reservation system, (2) IDM only models following behaviour on straight lane segments — it is not applied to the short connection lanes inside junctions, so cars do not brake for others already crossing. Fix requires a junction reservation or gap-acceptance model applied to `TRANSIT_INTERSECTION` agents before they are admitted onto a junction lane. | open |
