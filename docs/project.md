...
- **Building road attachment**: buildings store `(edge_idx, frontage_t: f32, side, cell_x, cell_y)`. Departure node is still computed on-demand via `building_depart_node(building, graph)` — returns `edge.start_node` when `frontage_t < 0.5`, otherwise `edge.end_node` — but the live FSM no longer walks directly to that node. `TRANSIT_DEPARTING` walks straight from the building interior toward a computed curb point on the frontage edge and inserts the agent mid-edge onto a matching sidewalk or vehicle lane when possible; `TRANSIT_ARRIVING` snaps from the frontage lane back to the curb point and then walks straight to the building center. No virtual split node is inserted into the road graph. `frontage_node` column removed from the `buildings` SQLite table. Detailed teardown, planned SoA migration, and redesign proposal: [`docs/entrance_and_exit.md`](entrance_and_exit.md).
