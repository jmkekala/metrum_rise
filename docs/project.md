...
- Extended `WorldEditor` with the first authored-water slice: bottom-toolbar `Water` subtools for `Source`, `Sink`, `Lake Fill`, and `Open Water`, plus `WorldDefinition` persistence for authored water boundary points, inland lake fills, and edge-connected open-water fills with immediate preview rebakes into the runtime water map. See [`terrain.md`](terrain.md) and [`ui.md`](ui.md).
