...
- **Canonical node queries**: editor node exports and nearest-node queries now ignore alias nodes left behind by `unite_nodes()`. The holographic node overlay and node-based tools therefore operate on one live canonical node per junction instead of showing or selecting merged ghost duplicates.
