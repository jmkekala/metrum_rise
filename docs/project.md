...
| B16 | [BUG] | `simulation/network/lanes/rebuild.rs:rebuild_edges_incremental` | Crosswalk geometry does not update when a junction changes degree (e.g. T→4-way). Root cause: `rebuild_edges_incremental` appends new pedestrian connection lanes (`edge_id == usize::MAX`) but never removes old ones — they are not tracked in `edge_lanes` so the edge-cleanup pass misses them. `emit_crosswalk_markings` iterates **all** entries in `lane_system.lanes`, so orphaned crosswalk lanes from the previous junction shape continue to render. **Workaround applied**: `create_edge_internal` now calls `lane_system.rebuild(graph)` (full O(E×N) rebuild) instead of the incremental path. This is correct but rebuilds all edges on every road commit. **Proper fix**: (1) Add `pub node_id: usize` to `Lane` (use `usize::MAX` for road lanes). (2) Add `pub node_lanes: HashMap<usize, Vec<usize>>` to `LaneSystem`. (3) In `build_pedestrian_connections_at_node` and `build_vehicle_connections_at_node`, record each new connection lane index in `node_lanes[node_id]`. (4) In `rebuild_edges_incremental`, before calling the two `build_*_connections_at_node` functions for each affected node, look up `node_lanes[node_id]`, set `lane.is_crosswalk = false` and `lane.geometry.clear()` on each listed lane (tombstones them for the renderer), then clear `node_lanes[node_id]`. Step (4) ensures old crosswalk lanes are invisible without requiring a Vec compaction. | workaround active — full rebuild on every road commit |
