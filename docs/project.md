...
74. **Single-file SQLite savegame (`savegame.sqlite`)**: implement save/load as one SQLite database written in a single transaction. Current-game schema example:
    - `save_meta(version, saved_at_unix, game_build)`
    - `map_config(width_m, height_m, env_cell_m, zone_cell_m)`
    - `time_state(time_elapsed, speed_multiplier, current_day, seconds_per_day, agent_sim_time)`
    ...
    - `lane_connections(node_id, from_edge, from_lane, to_edge, to_lane)`
    - `zoning_grids(edge_id, cells_long, left_zone_blob_u8, right_zone_blob_u8)`
    - `buildings(building_id, edge_id, frontage_t, side, cell_x, cell_y, zone_type, occupancy, width, depth)`
...
67. **Pedestrian junction links (crosswalks + explicit apron metadata)** — replace the current auto-derived node-local pedestrian transfers with authored crossing permissions and explicit per-link pedestrian junction data for the current road system. The simple straight-road + footpath curb-apron mesh is now generated automatically in the renderer; the remaining work is to make crossings and ambiguous apron joins data-driven instead of inferred from incident geometry.
   ...
   **Rendering** (`godot/scripts/crosswalk.gd`, new; `simulation/network/render/road.rs`): render crosswalk stripes from `Crosswalk` links, not "one quad per road arm". Each stripe instance spans one specific shoulder-to-shoulder crossing. The current Rust renderer already emits an automatic curb-apron patch for the simple straight road + footpath pass-through case; item 67 extends that to explicit `Apron` links so wider, offset, or otherwise ambiguous footpath-to-sidewalk joins are data-driven instead of guessed from the road geometry. Visuals rebuild whenever pedestrian-link data changes.
