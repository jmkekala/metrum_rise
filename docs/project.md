...
- FSM state names (entrance/exit Phase 3): `IN_BUILDING → ACCESS_EGRESS → NETWORK → ACCESS_INGRESS → IN_BUILDING` + `IMMIGRATING` and `INTERSECTION`. The live movement behavior is still the legacy frontage/curb system; only the authoritative names and persisted state contract have changed so far.
...
- **Building road attachment**: buildings still store authoritative `(edge_idx, frontage_t: f32, side, cell_x, cell_y)` and departure node is still computed on-demand via `building_depart_node(building, graph)` — returns `edge.start_node` when `frontage_t < 0.5`, otherwise `edge.end_node`. The live FSM is still the legacy frontage/curb system: `TRANSIT_ACCESS_EGRESS` walks straight from the building interior toward a computed curb point on the frontage edge and inserts the agent mid-edge onto a matching sidewalk or vehicle lane when possible; `TRANSIT_ACCESS_INGRESS` snaps from the frontage lane back to the curb point and then walks straight to the building center. No virtual split node is inserted into the road graph. Separately, `BuildingAllocator` now derives and caches exact entrance geometry from the asset-authored `entrance/main` anchor and live lane topology, but the movement FSM does not consume that cache yet. `frontage_node` column removed from the `buildings` SQLite table. Detailed teardown, planned SoA migration, and redesign proposal: [`docs/entrance_and_exit.md`](entrance_and_exit.md).
