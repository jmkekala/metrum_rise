...
- **Graph-dilation road renderer**: `RoadRenderer` no longer infers custom junction polygons from edge-local offsets. It now follows the `graph-road-renderer` model directly: emit a widened sidewalk-colored base from road-centerline strips, add circular node fills at true junctions and terminals, then draw the asphalt surface on top from the same strips and node disks. This removes the old contour solver and all edge-to-node handoff clipping from the active top-surface path.
- **Visible composition instead of contour ownership**: sidewalks are rendered first on the lower road layer, asphalt is rendered second on a slightly raised layer, and lane markings stay on the separate overlay mesh. The visible road is therefore the union of graph-dilation primitives and draw order, not a stitched `outer - road` contour solve. Arbitrary-angle bends, T-junctions, merges, and width transitions stay connected because the top surface is never rebuilt from pairwise offset intersections.
