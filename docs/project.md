...
- **Area-based road renderer**: `RoadRenderer` now trims every true multi-road junction by a node-owned handoff envelope and builds the junction itself from the node-local union of the trimmed road strips. Edge asphalt now hands off at the road boundary, edge sidewalks hand off later at the outer curb boundary, `Standard` / `Ramp` junction sidewalks are generated as one matching outer band, and straight 2-way pass-through splits are left as continuous ribbons instead of receiving a separate node patch.
- **Seam and shallow-angle continuity**: split straight roads stay coplanar without a blank center island, arbitrary-angle 2-way bends and 3-way junctions close by stitching the actual trimmed road caps instead of sampling a blob field, and 4-way nodes stay inside the road cross instead of inflating into diagonal lobes. This removes the visible road caps, broken curb joins, straight-road bubbles, and shallow-angle sidewalk artifacts that came from the old edge-local corner logic.
...
- **Junction Rendering (Item 70)**: Replaced the old corner-intersection / radial-fan approach with a node-local union solve over the trimmed road strips. The node patch now samples asphalt and sidewalk boundaries from one shared angular solve, so the junction keeps a continuous sidewalk band while edge sidewalks are cut before they can extend under node-owned asphalt. This keeps arbitrary-angle bends closed, avoids emitting asphalt islands on straight pass-through splits, and keeps 4-way nodes out of diagonal corner blobs.
