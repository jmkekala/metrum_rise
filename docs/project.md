...
- **Persistence and runtime**: SQLite save/load, background simulation thread, render snapshots, debug flags, asset editor, and economy editor are live. The asset editor now supports multi-part building assets, driveway/parking/loading-bay site anchors, and authored polygon yard surfaces for asphalt, concrete, gravel, and paving. Yard surfaces are visual-only and rendered from asset data; vehicle parking and freight stop behavior remain later runtime hooks.
