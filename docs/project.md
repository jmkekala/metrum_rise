...
- **Area-based road renderer**: `RoadRenderer` now treats every true multi-road junction as a node-owned 2D mask solve. For each junction, the renderer rasterizes a local `road_mask` and `outer_mask`, derives node sidewalk ownership as `outer - road`, triangulates the asphalt contour, and emits sidewalk quads only on the exposed outer hull. Edge asphalt and edge sidewalks stop at separate handoff distances before they enter that node-owned junction footprint.
- **Seam and shallow-angle continuity**: split straight roads stay coplanar without a blank center island, arbitrary-angle bends/merges are resolved by the same mask ownership rule instead of pairwise edge stitching, and 4-way nodes stay inside the actual road footprint instead of inflating into diagonal lobes. This removes the visible road caps, broken curb joins, straight-road bubbles, and shallow-angle sidewalk artifacts that came from the old edge-local corner logic.
...
- **Junction Rendering (Item 70)**: The old corner-intersection / radial-fan junction builder is gone from the active path. Junction ownership now comes from a node-local raster solve over trimmed road corridors, with asphalt filled from the road mask and sidewalks emitted only from the exposed `outer - road` mask. Straight pass-through splits remain pure edge ribbons, while arbitrary-angle bends, T-junctions, shallow merges, and 4-way nodes all render through the same node-local ownership contract.
