...
- **Persistence and runtime**: SQLite save/load, background simulation thread, render snapshots, debug flags, asset editor, and economy editor are live. The asset editor now supports multi-part building assets, driveway/parking/loading-bay site anchors, and authored polygon yard surfaces for textured asphalt and concrete. Yard surfaces are editor-preview metadata only for now; live terrain-integrated yards are parked as `EARTH-02`, and vehicle parking / freight stop behavior remain later runtime hooks.
