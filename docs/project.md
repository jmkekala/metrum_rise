...
- **Side-aware pedestrian routing graph (Item 68)**: `simulation/pathing/pedestrian.rs` now builds a thin query-time virtual pedestrian graph on top of `RegionGraph` instead of treating walking as a centerline-road query with a later visual offset. Sidewalk-capable road edges contribute left and right shoulder lanes, explicit `TransitType::Foot` edges contribute centered pedestrian lanes, and node-local pedestrian transfers are derived from the actual incident shoulder / footpath arm positions at query time. `AgentSystem` stores both a concrete pedestrian edge path (`current_ped_path`) and the active shoulder sign (`pedestrian_side`), walking mode selection uses the side-aware pedestrian query distance, and walking agents now follow recorded shoulder steps instead of re-inferring a sidewalk side from `bezier_t`. This is the live pedestrian-side foundation that future bicycle routing can reuse once item 30 adds `BIKE`-capable edges and mode selection.
...
- **Editor snapping**: `get_closest_network_point()` now projects edge snaps to the exact road centerline point on the hovered segment instead of quantizing them in 2 m steps. Node snapping still has absolute priority near existing junctions, but branch roads no longer depend on landing in the right quantized bucket to produce a clean split.
- **Sidewalk access matches routing flags**: the renderer no longer infers sidewalks from `EdgeClass`. Surface roads only get sidewalk strips and sidewalk node fills when `allowed_types & FOOT != 0`, and explicit `TransitType::Foot` walkways now participate in that same sidewalk pass instead of rendering as isolated strips. Simple branch cases use a sidewalk pass-through classification so the road-side sidewalks stay intact instead of being replaced by a shared sidewalk node disk. Duplicate-angle sidewalk boundary rays also collapse to the outermost point for the remaining polygon-fill cases. However, centered or across-road walkway joins are still incorrect because the renderer trims the footpath at the road-side sidewalk edge without generating the curb-apron patch needed to bridge into the shoulder geometry; see Known Bugs.
...
|----|------|-------------|----------|
| B8 | `simulation/network/render/road.rs` | `TransitType::Foot` walkways joining a sidewalk-carrying road can still punch visible triangular holes out of the road shoulders. The current centerline-road / implicit-sidewalk model trims the walkway back to the sidewalk edge but does not build the curb-apron geometry needed to connect the footpath into the road-side shoulder. Pedestrian pathing is now side-aware, but the renderer still lacks explicit apron geometry for these joins. | [BUG] |
...
67. **Pedestrian junction links (crosswalks + curb aprons)** — replace the current auto-derived node-local pedestrian transfers with authored crossing permissions and explicit curb-apron geometry for the current road system.

   **Data model** (`network/types.rs`, `network/graph/rebuild.rs`): do not use a node-wide `crosswalk: bool`. The current road system is centerline-based, road sidewalks are implicit shoulder geometry, and explicit `TransitType::Foot` walkways are real graph edges. The node therefore needs explicit pedestrian-arm links, not a single yes/no flag. Each sidewalk-capable road contributes two virtual pedestrian arms at the node (left shoulder, right shoulder). Each `TransitType::Foot` endpoint contributes one pedestrian arm. Store a compact per-node list of `PedestrianLink { from_arm, to_arm, kind }`, where `kind` is `Continue`, `Crosswalk`, or `Apron`. `Continue` links are auto-generated for same-side continuation through ordinary road nodes. `Apron` links are auto-generated where a footpath terminates into a sidewalk-carrying road node. `Crosswalk` links are the authored curb-to-curb permissions across asphalt. Rebuild and remap this data whenever topology changes or edges are compacted.

   **Rendering** (`godot/scripts/crosswalk.gd`, new; `simulation/network/render/road.rs`): render crosswalk stripes from `Crosswalk` links, not "one quad per road arm". Each stripe instance spans one specific shoulder-to-shoulder crossing. `Apron` links are not decals; they drive sidewalk join geometry in the Rust road renderer so footpaths connect into road shoulders without carving holes out of the sidewalk band. Visuals rebuild whenever pedestrian-link data changes.

   **Rust export** (`nodes/simulation_node.rs`): keep `get_crosswalk_transforms() -> PackedFloat32Array`, but generate one transform per `Crosswalk` link rather than per node/arm. Replace the node-wide toggle with a pair-based authoring API such as `toggle_crosswalk_link(node_id: i64, arm_a: i64, arm_b: i64)` so the tool can enable one specific crossing without changing every arm at the node. Also expose whatever compact pedestrian-link data the road renderer needs to build `Apron` joins deterministically.

   **Toggle tool** (`godot/scripts/crosswalk_tool.gd`, new): click a junction to preview its pedestrian arms, then click two opposite shoulders to toggle exactly one `Crosswalk` link. Footpath-to-sidewalk `Apron` links are automatic and not hand-authored. The highlight UI should show authored crossings per pair, not a node-wide on/off state.

   **Routing integration** (`simulation/pathing/pedestrian.rs`): the side-aware pedestrian graph from item 68 is already live, but today it auto-connects all incident pedestrian arms at a node by geometry. Item 67 replaces that fallback with explicit `Continue`, `Apron`, and authored `Crosswalk` links so the query only permits legal same-side continuation, footpath-to-shoulder entry, and curb-to-curb crossings.
