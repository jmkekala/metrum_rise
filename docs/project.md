...
- Replaced the legacy `MapConfig` type with chunk-aware `WorldConfig`, added terrain chunk metadata to saves, and removed the old `10 km` versus `20 km` gameplay startup split. Terrain and water storage are still dense runtime buffers for now; chunk metadata is the first migration slice.
