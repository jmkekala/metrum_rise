...
- `PORT-01`: the physical layer port, at `P0`. Water that flows, fire with a fuel bed, wind, minerals
   derived from formation, flora, and fauna as one combined field. It is the most downstream decision
   here, because everything built before it assumes still-water fills and absent fire. See [`simulation_layers.md`](simulation_layers.md).
- `TURN-01`: turn lanes and a turn hierarchy. Junction conflict resolution is live and right turns still
   collide, because the conflict table can say a path is occupied but not who had the better claim. See
   [`roads.md`](roads.md).
