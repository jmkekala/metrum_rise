...
apply_zoning_parcel_rezone_drag(start_x, start_z, end_x, end_z, selected_profile_runtime_id)
...
parcel when the target point is already inside one. Hover preview follows the same Rust-owned hit
test: hovering an existing parcel previews that parcel with the selected target profile color;
hovering empty road-adjacent land previews a new placement if legal. Dedicated select, delete,
merge, split, and bulk-edit commands are deferred until they have a gameplay/UI need.
...
- Godot uploads Rust-produced parcel render payloads; large drag previews use a packed corner array
  to avoid one dictionary allocation per preview parcel
- Godot may cache/throttle visual preview requests by small mouse movement, but commit calls must
  always go back through Rust placement
