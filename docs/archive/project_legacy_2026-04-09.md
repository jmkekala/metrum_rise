...
**Scale target**: ≥ 1,000,000 total population across one large world, with a clear distinction between simulation tiers:
...
- **Statistical tier** (aggregate population counters, demand flow numbers, no individual simulation): distant world regions outside the active area of interest. No per-tick FSM cost.
...
World scale is variable (default 20 km × 20 km, player-configurable) under one unified `RegionGraph`. Full-FSM simulation is reserved for the active area of interest; distant regions of the same world run at lower fidelity.
...
This file is the implementation ledger for the project: current shipped behavior, known bugs, and milestone backlog. Detailed subsystem specs live in dedicated documents such as [`docs/entrance_and_exit.md`](entrance_and_exit.md), [`docs/economy.md`](economy.md), [`docs/demand.md`](demand.md), and [`docs/zoning.md`](zoning.md).
...
- Need a spatial query? Use the existing spatial structure that matches the query: the road-edge R-tree, the 16 m node grid, the 512 m building/routing chunk indices, or `DataGrid<T>`.
...
- **Unified Pedestrian Navigation (`LaneSystem`)**: Pedestrians now traverse the same `LaneSystem` architecture as vehicles, using dedicated `LaneType::Foot` splines and crosswalk connection splines at junctions. 
    ...
    - **Legacy Purge**: `simulation/pathing/pedestrian.rs` and all its fallbacks have been removed. `AgentSystem` now uses a single `TRANSIT_NETWORK` state for both cars and pedestrians, distinguished only by the assigned lane types.
...
- **Frontage split removal (2026-04-06)**: `split_for_frontage`, `remove_frontage`, and `NodeType::Frontage` have been deleted. Buildings no longer insert virtual nodes into the road graph. The temporary `building_depart_node(building, graph)` endpoint proxy introduced during that cleanup has now also been removed by the entrance/exit redesign. Zone grid fragmentation caused by frontage splits is eliminated. Pedestrian junction crossing guard for `Frontage` nodes removed from `pedestrian_junctions.rs`.
- **Vehicle frontage access policy (2026-04-08, entrance/exit Phase 1)**: `Edge::vehicle_frontage_access: VehicleFrontageAccess` is now authoritative per-edge network state. Ordinary road creation defaults it to `BothSides`; walkway edges (`TransitType::Foot`) default it to `SameSideOnly`. Edge splits preserve the value, and topology merge refuses to merge two edges if their frontage-access policies disagree so the edit boundary is kept instead of inventing a merged value. Save/load persists it as `network_edges.vehicle_frontage_access INTEGER NOT NULL DEFAULT 1` with a forward-compatible `ALTER TABLE ADD COLUMN` migration that upgrades older saves to `BothSides`. The road/editor bridge now exposes `SimulationNode.set_vehicle_frontage_access(edge_idx, access_int)` and `get_vehicle_frontage_access(edge_idx)` (`0 = SameSideOnly`, `1 = BothSides`) so the value is not just serialized state. The live movement FSM does not consume this field yet; it is the locked network-side input for the entrance/exit rewrite in [`docs/entrance_and_exit.md`](entrance_and_exit.md).
...
- **Derived entrance cache (2026-04-08, entrance/exit Phase 2)**: `BuildingAllocator` now maintains a per-building derived `entrances` cache sourced from the asset-authored `entrance/main` anchor plus live road/lane topology. Each cache entry stores the parent `edge_idx`, building `side`, copied `vehicle_frontage_access`, exact `entrance_s_m`, world `door_pos`, world pedestrian `curb_pos`, valid frontage foot/car lane ids, and derived validity flags. The authored `main` entrance anchor is interpreted directly in lot-space meters; it is not multiplied by `preview_scale` or shifted by `pivot_offset`, which only affect mesh rendering. The cache now rebuilds eagerly after building-list mutations, after load (once transforms and lanes exist), after bulk-load lane finalization, and after road/lane editor operations that can change the attached edge or its legal frontage lanes. This cache is not yet consumed by the live agent FSM; it is the deterministic building-side input for the later entrance/exit movement rewrite in [`docs/entrance_and_exit.md`](entrance_and_exit.md).
- **Building Allocator (R10)**: [DONE 2026-04-06] The monolithic `allocator.rs` has been split into a modular directory structure:
    ...
    - `lifecycle.rs`: Stale building removal, immigration admission, and coordinate restoration.
...
- FSM state names (entrance/exit Phase 3): `IN_BUILDING → ACCESS_EGRESS → NETWORK → ACCESS_INGRESS → IN_BUILDING` + `IMMIGRATING` and `INTERSECTION`. The live movement loop now follows the entrance/exit model from `docs/entrance_and_exit.md` for ordinary trips. Daily household immigration no longer uses the `IMMIGRATING` bootstrap; admitted residents are instantiated directly inside their claimed home. `TRANSIT_IMMIGRATING` remains only as a transport-layer helper for exceptional/manual border-spawn cases.
- Movement: polyline traversal with sub-tick `remaining_dist` budget; lane offsets from road width / lane count. Cars now use IDM-based lane following, overlap correction, and a junction capacity gate before entering connection lanes. Pedestrians still use fixed-speed sidewalk/crosswalk traversal. Full microscopic IDM-through-junction behavior remains in the backlog.
- **Building road attachment**: buildings still store authoritative `(edge_idx, frontage_t: f32, side, cell_x, cell_y)`, but the live building/network attachment model is now the derived `BuildingEntrance` cache rather than any endpoint-node proxy. `BuildingAllocator` computes `door_pos`, `curb_pos`, legal frontage lanes, and `vehicle_frontage_access`-aware access metadata from the asset-authored `entrance/main` anchor plus live lane topology. `TRANSIT_ACCESS_EGRESS` and `TRANSIT_ACCESS_INGRESS` consume that cache and the exact `planned_attach_*` / `planned_detach_*` fields instead of rebuilding curb/frontage heuristics per tick, and freight ETA now uses the same cache on the building side for supplier and `OWA` route evaluation. No virtual split node is inserted into the road graph. `frontage_node` column removed from the `buildings` SQLite table. Detailed teardown, migration plan, and final target behavior: [`docs/entrance_and_exit.md`](entrance_and_exit.md).
- **Agent trip-plan SoA fields (2026-04-08, entrance/exit Phase 3)**: `AgentSystem` now carries the compact exact-trip/access scalars from [`docs/entrance_and_exit.md`](entrance_and_exit.md): `planned_attach_node`, `planned_detach_node`, `planned_attach_lane_id`, `planned_detach_lane_id`, `planned_attach_lane_d`, `planned_detach_lane_d`, `access_flags`, and `next_replan_time`. These fields are now the only authoritative trip-plan state for ordinary agent travel; the old duplicate `target_node` and `is_visible` SoA fields were removed in Phase 8.
- **Frontage delay planning cache (2026-04-08, entrance/exit Phase 4)**: `Lane` now carries a derived `frontage_delay_penalty_s` cache for exact building-access planning. `AgentSystem::tick()` updates it at a fixed 1.0 second cadence from aggregated live `MODE_CAR` speeds on each vehicle lane; foot lanes are always forced to `0.0`, and the cache honors multiple elapsed 1-second steps when a large `delta` arrives in one tick. This is a low-frequency building-adjacent congestion signal only: full graph path cost and the live movement FSM remain unchanged, and no trip planner consumes this cache yet before entrance/exit Phase 5.
- **Exact entrance trip planning (2026-04-08, entrance/exit Phase 5)**: building-origin departures now build one explicit trip plan from the entrance cache instead of using `building_depart_node()` as the primary trip abstraction. Departure planning populates `planned_attach_*` / `planned_detach_*`, chooses `MODE_WALK` vs `MODE_CAR` by exact total trip cost with deterministic tie-breaks, and folds in frontage-delay penalties only on the vehicle frontage segments. Same-edge car trips now also price a direct frontage-only lane segment when the exact attach point and exact detach point lie in forward order on the same legal lane, instead of forcing that case through a fake endpoint-wrap cost. Flow-field fast paths are now sourced from legal entrance endpoints per mode and are accepted only when they already agree with the chosen target building, detach node, and turn-valid path; otherwise CCH remains authoritative. `TRANSIT_IMMIGRATING` border-spawn agents still build the same destination-side exact plan from a border node when that exceptional transport helper is used, but daily household immigration no longer depends on it. The later Phase 6 and Phase 7 movement work now consumes that planner output end-to-end, so the old frontage/ingress compatibility seam is no longer part of the live ordinary-trip path.
- **Exact entrance movement (2026-04-08, entrance/exit Phase 6)**: for agents carrying a valid exact access plan, `TRANSIT_ACCESS_EGRESS` and `TRANSIT_ACCESS_INGRESS` now move along the exact planned local-access geometry instead of reconstructing curb/frontage heuristics per tick. `MODE_CAR` stays a car and `MODE_WALK` stays a character through both local-access phases, including the deterministic opposite-side `BothSides` crossover path on wide roads. `NETWORK` now uses the exact `planned_detach_lane_id` / `planned_detach_lane_d` handoff for valid plans, keeps `current_node` as a last-reached live-network anchor, and re-plans valid trips from their current network anchor with CCH-only destination-side updates instead of hiding the agent or dropping back into midpoint/frontage arrival snaps. Same-edge direct car trips now complete the opposite-side ingress crossover correctly because the local-access path follower prefers the furthest progressed matching segment at shared collinear vertices instead of oscillating forever between the curb and crossover segments, and opposite-side car egress now treats the exact lane endpoint as authoritative when that endpoint lies back on the crossover segment so cars do not bounce forever between the final lane point and the outer-crossing point. Car local-access handoffs now serialize exact lane attachment and detachment per lane per tick and require the target frontage slot to be clear, which prevents simultaneous household/workplace departures or arrivals from deadlocking several cars on the same frontage handoff point. The remaining compatibility branches from the pre-Phase-5 model were removed in Phase 7.
- **Legacy fallback purge (2026-04-08, entrance/exit Phase 7)**: the remaining pre-Phase-5 frontage/curb compatibility code has been removed from the ordinary agent movement loop. `TRANSIT_ACCESS_EGRESS`, `TRANSIT_NETWORK`, and `TRANSIT_ACCESS_INGRESS` no longer fall back to midpoint arrival checks, frontage-node reinsertion, node-only building attachment, or forced `MODE_CAR -> MODE_WALK` conversion on final arrival. The snapshot/debug render helpers no longer aim egress at `current_node` or ingress at building center; they use the exact planned local-access references instead. Old no-plan active-trip states are now treated as invalid and scrubbed back into exact states instead of replaying the retired movement model.
- **Final SoA cleanup (2026-04-08, entrance/exit Phase 8)**: `target_node` and `is_visible` have been removed from the authoritative agent SoA, render snapshot path, and SQLite schema. Live visibility now derives solely from transit state: `IN_BUILDING` is hidden, while `ACCESS_EGRESS`, `NETWORK`, `ACCESS_INGRESS`, `IMMIGRATING`, and `INTERSECTION` are visible. Save/load now persists only `current_node` plus the exact planned attach/detach fields for agent travel; pre-Phase-8 saves are intentionally incompatible because the old duplicate fields and compatibility shims were dropped as part of the purge.
...
- **Stuck Agent Scrub**: `AgentSystem::tick` still includes a safety pass that collapses invalid ordinary trips back into hidden `IN_BUILDING` state instead of leaving "field-ghost" agents active in the world. Visibility is no longer a stored bit; render suppression now follows transit state directly.
...
- **Parallel tick (item 19)**: `AgentSystem::tick` uses Rayon-backed parallel dispatch for the IDM speed pass and the main movement pass once the active agent count clears the small-city threshold; below that threshold it stays sequential to avoid worker spin overhead. Immigration assignment (which needs mutable `BuildingAllocator`) remains sequential in a post-parallel phase.
...
- **Unit tests** (`simulation/economy/agents/tests.rs`): `test_agent_fsm_planned_departure_lifecycle` verifies that economy-planned departures leave `TRANSIT_IN_BUILDING` and enter the live road/pathing FSM correctly.

#### Agent Rules
- **Immigration**: if at least one connected `NodeType::Border` exists, the allocator admits explicit immigrant households through that external connection. Each admitted household claims real vacant housing first, then one resident agent per household member is instantiated directly inside the claimed home with a shared `household_id`; there is no longer a live border-to-home bootstrap trip for ordinary household immigration.
- **Housing assignment**: immigrant households currently sample the residential/mixed vacancy index and claim one home building with enough free slots for the admitted household size.
- **Daily activity**: households consume stock and request replenishment on the daily economy pass, while idle agents choose work/home trips from decision-utility planning instead of following a hard-coded shop loop.
- **Happiness/money**: household budgets receive wages from building operating budgets, pay household utility and replenishment costs, and sync back into per-agent money views. Daily home recovery and pollution penalties still apply to agents.
...
- Shipment ETA, supplier choice, and `OWA` border-terminal choice now use the derived `BuildingEntrance` cache plus exact car-access frontage cost instead of the deleted `building_depart_node()` endpoint proxy, so freight routing follows the same building/network attachment model as ordinary car trips.
...
- The `TRANSIT_IN_BUILDING` branch no longer rolls RNG for daily essentials.
...
- The old hidden empty-city `50/25/25` demand floor and the later manual founding-placement tool are both gone. New cities now start from a one-time founding bootstrap: once the player has an external connection plus valid residential and commercial zoning, the first live daily tick seeds exactly one residential building and one commercial building.
...
- **Agent routing in `tick.rs`**: `TRANSIT_IN_BUILDING` activations for work/shop use flow fields if a valid field exists; fall back to CCH if not. `TRANSIT_NETWORK` with empty path follows the same priority. Home trips always use CCH (specific building destination required).
...
- **IDM parallel update** (O(A)): for each car in `TRANSIT_NETWORK`, binary-searches `lane_occupants` to find the bumper-to-bumper gap to the nearest car ahead. Applies `a = A_MAX × (1 − (v/v_max)⁴ − (s*/gap)²)` with `s* = S_MIN + v × T_HEAD`. New speeds written to a `new_speed` scratch buffer, then committed to `agents.speed`.
...
[DONE] **E7. Player-focused founding bootstrap** — removed the hidden empty-city `50/25/25` demand floor, removed the later manual founding-building placement tool, replaced both with a one-time automatic founding bootstrap that seeds exactly one residential building and one commercial building once the player has an external road connection plus valid residential and commercial zoning, removed the remaining automatic private-building scaffold that was still filling zoned frontage behind the player's back, kept `DemandSystem` only as live pressure telemetry instead of a construction trigger, allowed `OWA` import fallback to scale shipments down to what a startup store can actually afford, added an early-city immigration floor so the first founded town no longer stalls after one household, and reduced founding-household starter savings so nearby jobs activate on the first real daily economy pass instead of idling for weeks.
...
### v0.2 — scaling baseline, multi-modal foundation, and large-world area-of-interest simulation

Target: ~250k–500k agents with the first non-car transport mode live and the large-world area-of-interest architecture in place.
...
**Implementation order matters.** Items 73, 19, 18, 25, and 74 are complete (see Implemented Systems). Item 30 (bicycle) builds on the `transit_mode` and `allowed_mask` infrastructure already in place. Items 54–56 (large-world area-of-interest simulation) require CCH (item 31, v0.1 blocker) to be complete.
...
54. **Large-world area-of-interest simulation — distant-region statistical model**: each inactive world region is represented by ~15 numbers per tick: population, employment capacity, demand by zone type, and throughput per regional connection. Updated on a coarse schedule (~1/s game time, not per tick). No individual agent simulation runs in those distant regions. The `RegionGraph` (see item 31) still spans the full world; regions outside the active area of interest are carried as aggregate counters rather than full FSM agents.
55. **Large-world area-of-interest simulation — promotion/demotion across fidelity boundaries**: when an agent leaves the full-FSM area and enters a low-fidelity world region, it is demoted to a statistical entry in a queue attached to that connection (arrival time estimated from CCH path cost). When it re-enters a promoted region, it is restored as a full FSM agent near the corresponding boundary anchor. If the destination region remains inactive, it is absorbed into that region's statistical population counter. Border-node-style spawn/despawn is the direct predecessor of this system.
56. **Large-world area-of-interest simulation — overview view**: a coarse top-level view showing world partitions, connection throughput, and aggregate demand flow. No agent rendering at this zoom level — statistical flow numbers only. Moving the active area of interest promotes nearby regions to full simulation and demotes distant regions to lower-fidelity modes.
...
57. **Asset editor / importer** — see `docs/asset_editor.md` for the full design. Implementation order is strict: each step unblocks the next.
    ...
    **Step 5 — Building importer: [DONE 2026-04-05]**
    ...
    - Updated 2026-04-08: the editor now round-trips the authored `entrance/main` anchor as explicit local-space data, previews it as a draggable viewport sphere/gizmo, preserves additional non-main anchors when an existing asset is re-exported, and exposes a selectable/copyable import log with a `Copy All` action.
