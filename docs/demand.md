...
- it computes and persists `startup_support_factor`, `households_to_admit_today`,
...
- baseline fresh-map startup now flows through the same demand-owned startup-support rules instead
  of a hidden allocator-owned founding path
...
- startup-support tuning is authored in the top-level `startup_support` table in
  [`demand/growth_profiles.toml`](../demand/growth_profiles.toml)
- baseline fresh-map startup now uses this demand-owned startup-support path directly; there is no
  separate allocator-owned one-time founding placement rule in the live runtime
...
- shipped fresh-map startup uses the demand-owned `startup_support` contract; any future special
