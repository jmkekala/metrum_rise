...
This document owns the city's coarse growth pressure and the target demand-driven growth system: household admission and removal, private building spawn and despawn pressure, and building upgrade or downgrade pressure. It is still work in progress.
...
The live game now has a larger Rust-side `DemandSystem`, but it is still only a partial
implementation of the full growth controller this document defines.
...
- the live runtime now executes ordinary household admission from the demand-owned
  ...
  allocator
- it is still not the full authoritative owner of emigration execution, private building spawning,
  private building despawning, or building upgrades and downgrades
...
- the current system now owns household-admission pressure and the daily admitted-household count
- it still does not yet own the city's full building-change decisions
...
- `current DemandSystem`: the live Rust system that now computes baseline `DemandChannel`s plus
  demand-owned daily household-admission or removal outputs, but not the full building-change loop
- `target demand layer`: the future authoritative growth controller described by this document
...
- household removal is still not executed from the demand-owned `households_to_remove_today`
  output
...
- `DemandSystem` now owns the household-admission decision and daily admitted-household count
- allocator lifecycle code still owns founding bootstrap and most building-change decisions
...
- `rust/src/simulation/buildings/allocator/mod.rs::tick()` still hardwires part of the live growth-related execution order by running stale cleanup and founding bootstrap inside allocator-owned tick flow. The ordinary household-admission decision is now outside allocator tick, but bootstrap still needs to move to explicit startup/scenario ownership later.
