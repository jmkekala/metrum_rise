...
- `DemandSystem` stores three global counters only: `residential`, `commercial`, and `industrial`, each in the range `0..100`
- those counters are rebuilt from live economy state once per daily economy tick, then rebuilt again after the household and building economy pass finishes
- they are persisted through save/load as lightweight economy telemetry
- they are not currently a player-facing control surface and are not the authoritative owner of immigration, emigration, private building spawning, private building despawning, or building upgrades and downgrades

Current derived inputs:

- residential pressure reads from housing fill and open job pressure
- commercial pressure reads from household stock pressure and commercial building stock pressure
- industrial pressure reads from commercial stock pressure and industrial stock pressure

Short version:

- the current system measures broad R/C/I pressure
- it does not yet own the city's full growth decisions

## Terminology Conventions

This document uses the following terms consistently:

- `current DemandSystem`: the small live Rust system that only rebuilds coarse `residential`, `commercial`, and `industrial` counters
...
- `DemandSystem` provides coarse telemetry
