...
The country holds `100,000 km2` of land, roughly `125 km x 800 km` measured
along the U. The inland sea is not counted in that figure and holds `50,000`
to `75,000 km2`. Of the regions, roughly 20 are land along the U and 5 or 6
are mainly ocean. A land region averages `5,000 km2` and is sized from the
seed inside a `50 km` to `75 km` band; an ocean region runs `8,000` to
`15,000 km2`.

Region shape is drawn to the environment and is never square, unless the
ground is exceptionally flat and offers no natural feature to bind an edge
to. [`terrain.md`](terrain.md) owns the generator that draws them.
