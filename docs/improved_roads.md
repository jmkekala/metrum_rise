...
- `Terminal`, `Bend`, and `JunctionN` visual node pieces now route through the canonical
  ...
  terrain clip loops, and node earthwork roots are exported from the triangulated top mesh rather
  than from the retired post-overlay repair path.
- the legacy terminal U-shaped end-band helper is deleted in the hardcut. A future terminal cap
  must be generated by the same library-backed rail / contour path as the rest of the node mesh,
  not by reintroducing the retired endpoint patch helper.
...
- terrain is clipped by the same final road-owned top-surface footprint, not by a separate hull or
  render outline. For `Terminal`, `Bend`, and `JunctionN` this is the footprint exported from the
  canonical triangulated top mesh.
...
Future terminal-cap acceptance:

The legacy terminal end-band helper is deleted. If terminal cap ownership is restored, it must be
generated by the canonical rail / contour pipeline and must satisfy these rules:
...
5. For `Terminal`, route the one-mouth side-band ownership through the same canonical pipeline as
   bends and junctions. Do not restore the old U-shaped endpoint helper as a compatibility patch.
   Any future terminal cap must be a canonical owned contour with explicit height provenance.
