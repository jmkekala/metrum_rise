...
| `CODE-05` | `refactor`  | `open` | `P2`     | [`entrance_and_exit.md`](entrance_and_exit.md) | `rust/src/simulation/network/lanes/pedestrian_junctions.rs` duplicates a large 25-line block verbatim to generate reverse-direction crosswalks instead of using an abstraction. | The duplicated directional logic is replaced by a small helper closure or macro to generate both forward and reverse lanes. |
...
| `CODE-07` | `refactor`  | `open` | `P0`     | [`economy.md`](economy.md)                     | `rust/src/simulation/economy/demand.rs` (2100+ lines) and `households.rs` (1500+ lines) are overgrown state containers violating the 1,000-line single-responsibility limit. | Promote modules to `demand/mod.rs` and `households/mod.rs` and split distinct data definitions and algorithms into isolated sibling files. |
...
| `CODE-11` | `refactor`  | `open` | `P0`     | [`entrance_and_exit.md`](entrance_and_exit.md) | `rust/src/simulation/economy/agents/tick.rs` illegally uses `RefCell` for thread-local global state mutability in parallel Rayon execution, risking runtime crashes. | Eradicate `RefCell` global buffers and safely pipe a mathematically pure `&mut AgentContext` pool down through the parallel hot-path computation stack. |
